...
const List = ({product}) => {
    ...
    const addCart = () => {
        ...
        let  index = cart.findIndex(item => item.id == product._id)
        if(index == -1) 
            cart.push({count:1,id:product._id})
        else 
            cart[index].count += 
        setCookie('cart',cart)
    }
    ...
    const addFav = () => {
        ...
        let fav = cookies.fav || []
        let  index = fav.findIndex(item => item.id == product._id)
        if(index == -1) 
            fav.push({id:product._id})
        else 
            fav.splice(index,1)
        setCookie('fav',fav)
    }
        ...
        <div className="lprobox">
            ...
            <div className="lprobox__right">
                ...
                <div className="d-flex align-items-center lprobox__bottom">
                    ...
                    <button 
                        className="btn btn__bg lprobox__button" 
                        onClick={addCart}
                    >
